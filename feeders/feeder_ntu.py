...
class Feeder(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', random_choose=False, random_shift=False,
        ...
        data_mode: 'bone': use bone modality or not; 'motion': use motion modality or not
    ...
    def load_data(self):
        ...
        if self.split == 'train':
            if self.debug:
                self.data = npz_data['x_train'][0:600]
            else:
                self.data = npz_data['x_train']
            if self.debug:
                self.label = np.where(npz_data['y_train'][0:600] > 0)[1]
            else:
            ...
            if self.debug:
                self.data = npz_data['x_test'][0:600]
            else:
                self.data = npz_data['x_test']
            if self.debug:
                self.label = np.where(npz_data['y_test'][0:600] > 0)[1]
            else:
