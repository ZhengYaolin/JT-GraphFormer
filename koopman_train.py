...
def get_parser():
    ...
    parser.add_argument('--eval_epoch', type=int, default=60, help='the start epoch to save model (#iteration)')
    ...
    parser.add_argument('--freeze_weights', type=str2bool, default=True)
...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            ...
            if self.arg.freeze_weights:
                for name, param in self.model.named_parameters():
                    if name == 'K' or 'blocks.7' in name:
    ...
    def train(self, epoch, save_model=False):
        ...
        if epoch + 1 >= self.arg.save_epoch:
            ...
            torch.save(weights, self.arg.work_dir + '/' + str(epoch+1) + '.pt')
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            self.print_log('evaluating: loss: {:.4f}, top1: {:.2f}%, best_acc: {:.2f}%'.format(losses.avg, top1.avg,
                                                                                               self.best_acc))
    ...
    def start(self):
        ...
        if self.arg.run_mode == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.eval_interval == 0 or epoch + 1 >= self.arg.eval_epoch):
